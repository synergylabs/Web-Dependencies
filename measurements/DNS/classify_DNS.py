...
def match_strict_TLD(website, ns):
    w_sld = tldextract.extract(
        website, include_psl_private_domains=True).domain
...
def classify(domain_ns_all: dict) -> Tuple[dict, list]:
    ...
    for website_domain, ns_set in domain_ns_all.items():
        if website_domain not in website_ns_type:
            website_ns_type[website_domain] = {}
        ...
        for ns in ns_set:
            ...
            if website_domain == ns_domain:
                ...
                if ns_soa and website_domain_soa:
                    ...
                    if website_domain_soa[MNAME] != ns_soa[MNAME] or website_domain_soa[RNAME] != ns_soa[RNAME]:
            ...
            if ns_type == Ns_type.Unknown:
                ...
                utils.log_classify_result(
                    f"{website_domain}\t{ns_domain.rstrip('.')}\t{ns_type.name}\n")

            website_ns_type[website_domain][ns] = ns_type

    # 4. ns concentration >= 50
    for ns_domain, websites in unknown_ns_website.items():
        if len(websites) >= 50:
            for website_domain in websites:
                website_ns_type[website_domain][ns_domain] = Ns_type.Third
                utils.log_classify_result(
    ...
    return website_ns_type, list(ns_soa_all.values())
